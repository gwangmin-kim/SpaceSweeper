...
---
## 4. UI / UX 디자인
	예시 이미지는 ppt 슬라이드 형식으로 화면 내의 요소들을 구상한 이미지(좌측)와, 해당 이미지를 기반으로 Google Gemini Nanobanana로 생성한 컨셉 이미지(우측)를 함께 첨부하였다.
	이미지는 단순히 예시에 불과하며, 반드시 실제 게임 내 화면의 분위기를 반영하는 것은 아니다. 오히려 현재는 캐주얼한 톤으로 생각 중에 있기에, 컨셉 이미지와 많이 방향성이 달라질 수 있음.
### 4.1. 메인 화면
- **게임 로고**
- **4개의 버튼**
	1. 새 게임
